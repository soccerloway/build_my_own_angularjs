...
describe('parse', function() {
	...
	it('can parse an floating point number', function() {
		var fn = parse('4.2');

		expect(fn()).toBe(4.2);
	});

	it('can parse an floating point number without an integer part', function() {
		var fn = parse('.42');
	...
	it('can parse scientific notation with float coefficient', function() {
		var fn = parse('.42e2');
		expect(fn()).toBe(42);
	});
	...
	// 应用安全相关 禁止访问 window constructor 等属性
	...
	it('short-circuites AND', function() {
		var invoked;
