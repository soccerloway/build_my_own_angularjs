...
describe('Scope', function() {
	...
	describe('digest', function() {
		...
		it('calls this watch function with the scope as the argument', function() {
		...
		/* 允许在digest循环中($watchfn和listenerFn中删除watcher) */
		...
		// 引入 parse后，对表达式的支持
		it('accepts expressions for watch functions in $watch', function() {
			var theValue;
			scope.aValue = 42;
		...
		it('accepts expressions for watch functions in $watchCollection', function() {
			var theValue;
			scope.aColl = [1, 2, 3];
		...
		// 当watches常量时，dirty-check不需要去检测这些常量, 在第一次执行listenerFn后
		// 立刻remove掉watcher
		...
		it('executes given function later in the same cycle', function() {
			...
			scope.$watch(
				...
				function(newValue, oldValue, scope) {
					...
					scope.asyncEvaluatedImmediately = scope.asyncEvaluatedImmediately;
				}
			);
	...
	// 此方法能够合并短时间内(50ms)的$digest循环，提高性能
		...
		_.forEach(['$emit', '$broadcast'], function(method) {
			...
			it('does not stop on exceptions on ' + method, function() {
				var listener1 = function(event) {
					throw 'listener1 throwing an exceptions';
				};
